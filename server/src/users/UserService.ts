...
const salesforceUserToUserModel = (record: any): User => {
    const user: User = {
    ...
    if (UserType[record.userType__c] === UserType[UserType.Educator]) {
        ...
        (user as Volunteer).employerName = record.employerName__c;
...
export const getVolunteers = async (limit: number, offset: number): Promise<User[]> => {
    ...
    await conn.query(
        ...
        function(err, result) {
            ...
            volunteers = result.records.map(record => salesforceUserToUserModel(record));
        }
    );
