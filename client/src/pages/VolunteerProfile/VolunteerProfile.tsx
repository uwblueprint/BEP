...
> {
  ...
  componentDidMount() {
    this.props.fetchActiveEvents(this.props.userType, this.props.userId);

    const volunteer: Volunteer = this.props.location.state.volunteer;
    this.props.fetchApplications(volunteer.id);
    this.props.fetchInvitations(volunteer.id);
  ...
  render() {
    let appIds = this.props.applications.map(
      (app: Application) => app.event.id
    );
    let inviteIds = this.props.invitations.map(
      (invite: Invitation) => invite.event.id
    );
          ...
          <Grid item xs={12}>
            <PageBody>
              <div style={{ paddingBottom: "30px", paddingTop: "30px" }}>
                <AboutCard
                  description={
                    this.props.location.state.volunteer.careerDescription
                  }
                  jobTitle={this.props.location.state.volunteer.jobTitle}
                  employmentStatus={
                    this.props.location.state.volunteer.phoneNumber
                  }
                ...
                <ExpertiseCard
                  expertiseAreas={
                    this.props.location.state.volunteer.expertiseAreas
                  }
                  postSecondaryTraining={
                    this.props.location.state.volunteer.postSecondaryTraining
                  }
                  languages={this.props.location.state.volunteer.languages}
                  extraDescription={
                    this.props.location.state.volunteer.extraDescription
                  }
                />
                <ActivitiesCard
                  internalActivities={
                    this.props.location.state.volunteer
...
const mapStateToProps = (state: any) => {
  ...
  return {
    ...
    invitations: getVolunteerInvitations(state.volunteers),
  };
};
...
const mapDispatchToProps = (dispatch: any) => ({
  ...
  fetchApplications: (id: string) => dispatch(fetchApplicationsByVolunteer(id)),
  fetchInvitations: (id: string) => dispatch(fetchInvitationsByVolunteer(id)),
